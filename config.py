...
DEFAULTS = {
    ...
    #     "ai researcher", "deep learning", "nlp engineer", "llm", "generative ai",
    ...
    #"growing support", , "growing workload"
    "WORKLOAD_SIGNALS": [
        # Customer support load
        "support load", "ticket volume", "support volume", "high volume",
        "customer queries",  "support demand",
        ...
        "increasing demand", "workload spike",
        "load spike", "surge", "ramp up",
        # Urgency
        "immediately", "urgently", "asap", "as soon as possible",
        "urgent hire","urgent need","urgent", "start asap", "immediate start",
    ],
    #"company is growing", "join a growing team",
    # ── STEP 4: Vague/generic signals that are NOT valid workload evidence ────
    "VAGUE_SIGNALS": [
        "we are scaling", "scaling team",
    ...
    # ── API Keys (user-configured — update via Settings page or directly here) ─
    "OPENAI_API_KEY":                 os.getenv("OPENAI_API_KEY"),   # OpenAI API key for Structured Extraction AI + Sales Outreach
    "OPENAI_MODEL":                   os.getenv("OPENAI_MODEL"),  # cost-effective default
    "SERPAPI_KEY":                    os.getenv("SERPAPI_KEY"),   # SerpApi key for Google Jobs search
    "GOOGLE_SHEET_ID":                os.getenv("GOOGLE_SHEET_ID"),    #https://docs.google.com/spreadsheets/d/1kRjEWBYhsGnH4vyZcMpVgOW4spyfCQBy58EbX54E92A/edit?gid=0#gid=0 # Google Sheets spreadsheet ID
    "GOOGLE_SHEETS_CREDENTIALS_JSON": os.getenv("GOOGLE_SHEETS_CREDENTIALS_JSON"),   # path to service account JSON file
}
